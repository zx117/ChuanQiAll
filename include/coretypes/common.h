...
/*!
 * @namespace daq
 * The cqDAQ namespace. All interfaces and type definitions are within the
 * `daq` namespace.
 */
#define BEGIN_NAMESPACE_CQDAQ \
    namespace daq          \
    {

#define END_NAMESPACE_CQDAQ \
    }

#define BEGIN_NAMESPACE_CQDAQ_MODULE(name) \
    namespace daq::modules::name         \
    {

#define END_NAMESPACE_CQDAQ_MODULE \
    }

#define BEGIN_NAMESPACE_CQDAQ_SEARCH \
    namespace daq::search \
    {

#define END_NAMESPACE_CQDAQ_SEARCH \
    }
...
#if defined(CQDAQ_SKIP_DLL_IMPORT)
...
#if defined(_MSC_VER)
    #define CQDAQ_NO_VTABLE __declspec(novtable)
    #define DAQ_EMPTY_BASES __declspec(empty_bases)
#else
    #define CQDAQ_NO_VTABLE
...
#define CQDAQ_INTERFACE(...) __VA_ARGS__
...
#define DECLARE_CQDAQ_INTERFACE_EX(interfaceName, baseInterface) \
    struct interfaceName;                                     \
                                                              \
    namespace InterfaceBase                                   \
    {                                                         \
        struct CQDAQ_NO_VTABLE DAQ_EMPTY_BASES interfaceName##Impl : public baseInterface     \
        {                                                     \
            ...
            using Actual = interfaceName;                     \
                                                              ...
                                                              \
    struct CQDAQ_NO_VTABLE DAQ_EMPTY_BASES interfaceName : public InterfaceBase::interfaceName##Impl

#define DECLARE_CQDAQ_INTERFACE(interfaceName, baseInterface) \
    struct interfaceName;                                  \
                                                           \
    namespace InterfaceBase                                \
    {                                                      \
...
#define DECLARE_TEMPLATED_CQDAQ_INTERFACE_T(interfaceName, baseInterface) \
    template <typename T>                                              \
    struct interfaceName;                                              \
                                                                       \
    namespace InterfaceBase                                            \
...
#define DECLARE_TEMPLATED_CQDAQ_INTERFACE_T_U(interfaceName, baseInterface) \
...
#define DECLARE_CQDAQ_CUSTOM_INTERFACE(interfaceName, baseInterface, namespaceName) \
    struct interfaceName;                                                        \
                                                                                 \
    namespace InterfaceBase                                                      \
    {                                                                            \
...
#if defined(CQDAQ_DISABLE_CURRENT_FUNCTION)
    #define CQDAQ_CURRENT_FUNCTION "Object"
#elif defined(__GNUC__) || (defined(__MWERKS__) && (__MWERKS__ >= 0x3000)) || (defined(__ICC) && (__ICC >= 600)) || defined(__ghs__)
    #define CQDAQ_CURRENT_FUNCTION __PRETTY_FUNCTION__
#elif defined(__DMC__) && (__DMC__ >= 0x810)
    #define CQDAQ_CURRENT_FUNCTION __PRETTY_FUNCTION__
#elif defined(__FUNCTION__)
    #define CQDAQ_CURRENT_FUNCTION __FUNCTION__
#elif (defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 600)) || (defined(__IBMCPP__) && (__IBMCPP__ >= 500))
    #define CQDAQ_CURRENT_FUNCTION __FUNCTION__
#elif defined(__BORLANDC__) && (__BORLANDC__ >= 0x550)
    #define CQDAQ_CURRENT_FUNCTION __FUNC__
...
#define CQDAQ_TYPE_TOSTRING                                             \
    static daq::ErrCode CqDaqType(daq::CharPtr* str)                    \
    {                                                                     \
...
#define CQDAQ_INTFID(interfaceName, ...) CUSTOM_INTFID(interfaceName, "daq", ##__VA_ARGS__)

#if defined(NDEBUG) || defined(DS_CI_RUNNER)
    #define DEFINE_INTFID(interfaceName, ...)                                                           \
        static constexpr daq::IntfID Id = CQDAQ_INTFID(interfaceName, ##__VA_ARGS__)                  \
        CQDAQ_TYPE_TOSTRING
