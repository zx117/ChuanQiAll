...
#include <cqdaq/component_deserialize_context_factory.h>
...
ErrCode FolderImpl<Intf, Intfs...>::setActive(Bool active)
{
    ...
    if (CQDAQ_FAILED(err) || err == CQDAQ_IGNORED)
        return err;
...
CQDAQ_REGISTER_DESERIALIZE_FACTORY(StandardFolder)
