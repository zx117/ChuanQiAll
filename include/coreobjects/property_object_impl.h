                                                                                          ...
                                                                                          BaseObjectPtr& value)
{
    ...
    if (!prop.assigned())
    {
        return this->makeErrorInfo(CQDAQ_ERR_NOTFOUND, fmt::format(R"(Property "{}" does not exist)", name));
    }
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::checkContainerType(const PropertyPtr& prop, const BaseObjectPtr& value)
{
    ...
    if (coreType == ctObject)
    {
        ...
        return this->makeErrorInfo(CQDAQ_ERR_INVALIDTYPE, "Only base Property Object object-type values are allowed");
    }
    ...
    if (coreType == ctDict)
    {
        ...
        {
            return this->makeErrorInfo(CQDAQ_ERR_INVALIDTYPE, "Invalid dictionary key type");
        }

        dict->getValues(&it);
        if (!iterate(it, itemType))
        {
            return this->makeErrorInfo(CQDAQ_ERR_INVALIDTYPE, "Invalid dictionary item type");
        }
    }
    ...
    if (coreType == ctList)
    {
        ...
        {
            return this->makeErrorInfo(CQDAQ_ERR_INVALIDTYPE, "Invalid list item type");
        }
    }
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::checkStructType(const PropertyPtr& prop, const BaseObjectPtr& value)
{
    ...
    if (!structPtr.assigned())
        return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, "Set value is not a struct");
    ...
    if (structType != valueStructType)
        return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, "Set value StructureType is different from the default.");
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::checkEnumerationType(const PropertyPtr& prop, const BaseObjectPtr& value)
{
    ...
    if (!enumerationPtr.assigned())
        return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, "Set value is not an enumeration");

    auto propEnumerationPtr = prop.getDefaultValue().asPtrOrNull<IEnumeration>();
    if (!propEnumerationPtr.assigned())
        return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, "Property default value is not an enumeration");
    ...
    if (propEnumerationType != valueEnumerationType)
        return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, "Set value EnumerationType is different from the default.");
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::checkSelectionValues(const PropertyPtr& prop, const BaseObjectPtr& value)
{
    ...
    if (selectionValues.assigned())
    {
        ...
        return this->makeErrorInfo(CQDAQ_ERR_NOTFOUND, "Value is not a key/index of selection values.");
    }
                                                                                             ...
                                                                                             bool isUpdating)
{
    ...
    try
    {
        ...
        if (!prop.assigned())
        {
            return this->makeErrorInfo(CQDAQ_ERR_NOTFOUND, fmt::format(R"(Property "{}" not found.)", propName));
        }
    ...
    try
    {
        ...
        if (propCoreType != valueCoreType)
        {
            if (propCoreType == ctEnumeration)
            {
                ...
                if (!enumVal.assigned())
                    return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, fmt::format(R"(Default value of enumeration property {} is not assigned)", prop.getName()));
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::readLocalValue(const StringPtr& name, BaseObjectPtr& value) const
{
    ...
    if (it != propValues.cend())
    {
        if (info.index != -1)
        {
            ...
            {
                return this->makeErrorInfo(CQDAQ_ERR_INVALIDPARAMETER, "Could not access the index as the value is not a list.");
            }

            ListPtr<IBaseObject> list = it->second;
            if (info.index >= (int) list.getCount())
            {
                return this->makeErrorInfo(CQDAQ_ERR_OUTOFRANGE, "The index parameter is out of bounds of the list.");
            }
    ...
    return this->makeErrorInfo(CQDAQ_ERR_NOTFOUND, fmt::format(R"(Property value "{}" not found)", name));
}
                                                                                                ...
                                                                                                bool triggerEvent)
{
    ...
    {
        return this->makeErrorInfo(CQDAQ_ERR_NOTFOUND, fmt::format(R"(Property "{}" does not exist)", propName));
    }
    ...
    if (res != CQDAQ_ERR_NOTFOUND && CQDAQ_FAILED(res))
    {
        return res;
    }
    daqClearErrorInfo();

    if (res == CQDAQ_ERR_NOTFOUND)
    {
        this->clearErrorInfo();

        res = property->getDefaultValue(&value);

        if (CQDAQ_FAILED(res) || !value.assigned())
        {
            value = nullptr;
            daqClearErrorInfo();
            return CQDAQ_SUCCESS;
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::getPropertySelectionValue(IString* propertyName, IBaseObject** value)
{
    ...
    try
    {
        ...
        {
            return this->makeErrorInfo(CQDAQ_ERR_INVALIDTYPE, "List item type mismatch");
        }
    ...
    for (const auto& val : propValues)
    {
        ...
        if (ct == ctList || ct == ctDict)
        {
            if (const auto cloneable = val.second.asPtrOrNull<ICloneable>(); cloneable.assigned())
            {
                ...
                if (CQDAQ_FAILED(err) || !obj.assigned())
                    continue;
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::addProperty(IProperty* property)
{
    ...
    return daqTry([&]() -> auto {
        ...
        if (!propName.assigned())
            return this->makeErrorInfo(CQDAQ_ERR_INVALIDVALUE, "Property does not have an assigned name.");

        if (hasDuplicateReferences(propPtr))
            return this->makeErrorInfo(CQDAQ_ERR_INVALIDVALUE, "Reference property references a property that is already referenced by another.");

        propPtr.asPtr<IOwnable>().setOwner(objPtr);

        const auto res = localProperties.insert(std::make_pair(propName, propPtr));
        if (!res.second)
            return this->makeErrorInfo(CQDAQ_ERR_ALREADYEXISTS, fmt::format(R"(Property with name {} already exists.)", propName));
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::removeProperty(IString* propertyName)
{
    ...
    if(localProperties.find(propertyName) == localProperties.cend())
    {
        ...
        return this->makeErrorInfo(CQDAQ_ERR_NOTFOUND, fmt::format(R"(Property "{}" does not exist)", namePtr));
    }
...
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::toString(CharPtr* str)
{
    if (str == nullptr)
    {
        return this->makeErrorInfo(CQDAQ_ERR_ARGUMENT_NULL, "Parameter must not be null");
    }
                                                                                           ...
                                                                                           const ListPtr<IProperty>& props)
{
    ...
    for (const auto& prop : props)
    {
        ...
        if (!serializedProps.assigned() || !serializedProps.hasKey(propName))
        {
            ...
            if (!CQDAQ_SUCCEEDED(err) && err != CQDAQ_ERR_INVALID_OPERATION)
            {
                return err;
            }
...
CQDAQ_REGISTER_DESERIALIZE_FACTORY(PropertyObjectImpl)
