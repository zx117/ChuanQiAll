...
DECLARE_CQDAQ_INTERFACE(IStreaming, IBaseObject)
{
    /*!
     ...
     * @retval CQDAQ_ERR_DUPLICATEITEM if a signal on the list is already added to the Streaming.
     * @retval CQDAQ_ERR_NOINTERFACE if a signal on the list is not a mirrored signal.
     *
     ...
     */
    virtual ErrCode INTERFACE_FUNC addSignals(IList* cq_signals) = 0;

    // [elementType(signals, ISignal)]
    /*!
     * @brief Removes signals from the Streaming.
     * @param signals The list of signals to be removed.
     * @retval CQDAQ_ERR_NOTFOUND if a signal on the list was not added to the Streaming.
     *
     * After a signal is removed from the Streaming, the Streaming is automatically excluded in the list of
     * available streaming sources of a signal.
     */
    virtual ErrCode INTERFACE_FUNC removeSignals(IList* cq_signals) = 0;
