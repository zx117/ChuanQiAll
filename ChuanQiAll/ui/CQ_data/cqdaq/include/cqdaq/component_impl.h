...
#include <cqdaq/deserialize_component_ptr.h>
#include <coreobjects/core_event_args_impl.h>
#include <cqdaq/recursive_search_ptr.h>
#include <cqdaq/component_private_ptr.h>
#include <cqdaq/tags_impl.h>
#include <cctype>
#include <cqdaq/ids_parser.h>
#include <cqdaq/component_status_container_impl.h>
...
{
    CQDAQ_PARAM_NOT_NULL(statusContainer);
...
{
    CQDAQ_PARAM_NOT_NULL(attributes);
...
{
    CQDAQ_PARAM_NOT_NULL(outComponent);
    ...
    return daqTry(
        [&]()
        {
            ...
            *outComponent = findComponentInternal(this->template borrowPtr<ComponentPtr>(), str).detach();
            ...
            return *outComponent == nullptr ? CQDAQ_NOTFOUND : CQDAQ_SUCCESS;
        });
}
...
{
    CQDAQ_PARAM_NOT_NULL(parameter);
...
CQDAQ_REGISTER_DESERIALIZE_FACTORY(StandardComponent)
