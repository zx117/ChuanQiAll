...
public:
    ...
    ErrCode INTERFACE_FUNC getItemType(CoreType* type) override
    {
        ...
        {
            return makeErrorInfo(CQDAQ_ERR_GENERALERROR, e.what());
        }
        catch (...)
        {
    ...
    ErrCode INTERFACE_FUNC getOnPropertyValueWrite(IEvent** event) override
    {
        if (event == nullptr)
        {
            return makeErrorInfo(CQDAQ_ERR_ARGUMENT_NULL, "Cannot return the event via a null pointer.");
        }
    ...
    {
        if (!name.assigned() || name == "cqdaq_unassigned")
        {
            name = "cqdaq_unassigned";
            return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, "Property name is not assigned");
        }
        ...
        if (valueType == ctFunc || valueType == ctProc)
        {
            ...
            {
                return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE,
        ...
        {
            return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, fmt::format(R"(Property {} is missing its default value)", name));
        }
        ...
        if (refProp.assigned())
        {
            ...
            if (!valid)
                return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, fmt::format(R"(Reference property {} has invalid metadata.)", name));
        }
        ...
        if (valueType == ctStruct)
        {
            ...
            if (!valid)
                return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, fmt::format(R"(Structure property {} has invalid metadata.)", name));
        }
        ...
        if (valueType == ctEnumeration)
        {
            ...
            if (!valid)
                return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, fmt::format(R"(Enumeration property {} has invalid metadata.)", name));
        }

        // TODO: Make callable info serializable
        // if ((valueType == ctProc || valueType == ctFunc) && !callableInfo.assigned())
        //{
        //    return this->makeErrorInfo(CQDAQ_ERR_INVALIDSTATE, fmt::format(R"(Function- and procedure-type property {} must have
    ...
    ErrCode INTERFACE_FUNC setOwner(IPropertyObject* owner) override
    {
        ...
        {
            return makeErrorInfo(CQDAQ_ERR_ALREADYEXISTS, "Owner is already assigned.");
        }
...
CQDAQ_REGISTER_DESERIALIZE_FACTORY(PropertyImpl)
