 ...
 *
 * Most of cqDAQ's methods in interfaces return `ErrCode`. This is just an integer number. With `IErrorInfo`
 ...
 * {
 *     if (value < 0)
 *         return makeErrorInfo(CQDAQ_ERR_INVALIDPARAMETER, "Parameter should be >= 0", nullptr);
...
DECLARE_CQDAQ_INTERFACE(IErrorInfo, IBaseObject)
{
    /*!
...
CQDAQ_DECLARE_CLASS_FACTORY(LIBRARY_FACTORY, ErrorInfo)
