...
struct IUnknown
{
     ...
     * to the interface, while also calling `addRef`. Otherwise, it returns the CQDAQ_ERR_NOINTERFACE error code.
     *
     * Example:
     * @code
     * IUnknown* obj = ...;
     *
     * IBaseObject* baseObj;
     * auto errCode = obj->queryInterface(IBaseObject::Id, reinterpret_cast<void**>(&baseObj)), CQDAQ_SUCCESS);
 ...
 * comparison. All cqDAQ objects implement `IBaseObject` interface or its descendants.
...
struct IBaseObject : public IUnknown
{
     ...
     *
     * IBaseObject* baseObj;
     * auto errCode = obj->borrowInterface(IBaseObject::Id, reinterpret_cast<void**>(&baseObj)), CQDAQ_SUCCESS);
     * if (CQDAQ_FAILED(errCode))
     *     throw SomeException();
     * // ... do something with baseObj
     ...
     * @return CQDAQ_SUCCESS if succeeded, error code if failed.
     *
...
CQDAQ_DECLARE_CLASS_FACTORY(LIBRARY_FACTORY, BaseObject)
