...
namespace framework
{
     ...
     * used to simplify read, process and/or write channel data from/to chuanqi.
     */
    ...
    public:
        ...
        struct ProcessingContext
        {
            ...
            std::map<uint64_t, cq::framework::StreamIterator> m_channel_iterators;
        ...
        struct InitParams
        {
            ...
            const std::vector<cq::Property>& m_properties;
        };
    ...
    public:
         ...
         */
        virtual void create(cq::IfHost* host) { CQ_UNUSED(host); };
        ...
        virtual void updateInputChannelIDs(
            const std::map<std::uint64_t, std::uint64_t>& channel_mapping) { CQ_UNUSED(channel_mapping); };
         ...
         */
        virtual bool configure(const cq::UpdateChannelsTelegram& request,
            std::map<uint32_t, uint32_t>& channel_id_map)
            = 0;

        virtual void updatePropertyTypes(const PluginChannelPtr& output_channel) { CQ_UNUSED(output_channel); }

        virtual void updateStaticPropertyConstraints(const PluginChannelPtr& channel) { CQ_UNUSED(channel); }
         ...
         */
        virtual void initTimebases(cq::IfHost* host) { CQ_UNUSED(host); }

        /**
         * Called once before acquisition start
         * used to set up everything necessary for subsequent processing
         */
        virtual void prepareProcessing(cq::IfHost* host) { CQ_UNUSED(host); }

        /**
         * Called once after acquisition stop
         * used to clean up leftovers of processing
         */
        virtual void stopProcessing(cq::IfHost* host) { CQ_UNUSED(host); }
         ...
         */
        virtual void process(ProcessingContext& context, cq::IfHost* host) = 0;
    ...
    public:
         ...
         */
        std::map<uint64_t, cq::framework::StreamIterator> getSamplesAt(double time);
        ...
        void enqueueRequest(std::uint16_t id, const cq::PropertyList& params);
    ...
    protected:
         ...
         */
        virtual void init(
            const std::vector<InputChannel::InputChannelData>& input_channel_data) { CQ_UNUSED(input_channel_data); };
         ...
         *
         */
        virtual bool setup(const std::vector<cq::Property>& properties) { CQ_UNUSED(properties); return true; }
         ...
         */
        bool checkInputChannelFormats(const std::set<cq::ChannelDataformat::SampleValueType>& allowed_value_types,
         ...
         */
        void configureFromTelegram(const cq::UpdateChannelsTelegram& request,
                                   std::map<uint32_t, uint32_t>& channel_id_map);
         ...
         * @param channel_id_map  to fill mapping into
         */
        void createMappingByKey(const cq::UpdateChannelsTelegram& request,
                                std::map<uint32_t, uint32_t>& channel_id_map);
         ...
         * @param request  request telegram to use
         */
        void createChannelsFromTelegram(const cq::UpdateChannelsTelegram& request, std::map<uint32_t, uint32_t>& channel_id_map);
    ...
    private:
        ...
        void setupDataRequest(cq::IfHost* host);

        void updateInternalInputChannelIDs(const std::map<std::uint64_t, std::uint64_t>& changed_ids);

        bool handleConfigChange();

        void onInitTimebases(cq::IfHost* host, std::uint64_t token) final;

        void onStartProcessing(cq::IfHost* host, std::uint64_t token) final;

        void onStopProcessing(cq::IfHost* host, std::uint64_t token) final;

        void onProcess(cq::IfHost* host, std::uint64_t token, const cq::IfXMLValue* param) final;

        void onChannelConfigChanged(cq::IfHost* host, std::uint64_t token) final;

        std::map<uint64_t, cq::framework::StreamIterator> createChannelIterators(
        ...
        std::map<uint64_t, cq::framework::StreamIterator> createChannelIterators(
            const std::vector<cq::StreamDescriptor>& stream_descriptor,
            const cq::IfDataBlockList* block_list,
            const cq::Interval<double>& covered_interval,
    ...
    private:
        ...
        std::vector<const cq::IfDataBlockList*> m_block_lists;
