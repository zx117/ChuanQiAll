...
#include "api_software_channel_xml.h"
#include "api_utils.h"
...
namespace framework
{
    ...
    public:
        ...
        virtual bool validateInputChannels(const std::vector<InputChannel::InputChannelData>& input_channel_data,
            ...
            CQ_UNUSED(input_channel_data);
            CQ_UNUSED(invalid_channels);
            return true; 
        }
    ...
    protected:
        ...
        virtual cq::RegisterSoftwareChannel getSoftwareChannelInfo() = 0;

        PluginChannelsPtr getPluginChannels();
        std::shared_ptr<cq::framework::CustomRequestHandler> getCustomRequestHandler();

        bool checkChuanQiCompatibility();

    private:
        PluginChannelsPtr m_plugin_channels;
        std::shared_ptr<cq::framework::CustomRequestHandler> m_custom_requests;
    };
    ...
    protected:
        using SoftwareChannelInstanceRequestFunction = std::function<std::uint64_t(SoftwareChannelInstance* instance, const cq::PropertyList& params, cq::PropertyList& returns)>;
    ...
    private:
        ...
        template<typename T>
        typename std::enable_if< has_advanced_init<T>::value, cq::RegisterSoftwareChannel >::type
        ...
        cq::RegisterSoftwareChannel getSoftwareChannelInfo() final
        {
        ...
        bool createInstancesfromTelegram(
            const cq::UpdateChannelsTelegram& request,
        ...
        bool mapRemovedChannels(std::map<std::uint64_t, std::uint64_t>& mapped_channels)
        {
            ...
            auto channel_ids_xml = getHost()->template getValue<cq::IfXMLValue>(cq::queries::ChuanQiChannels, cq::queries::ChuanQiChannels_AllIds);

            if (channel_ids_xml)
            {
                cq::ChannelList channel_list;
        ...
        bool handleMessage(cq::PluginMessageId id, std::uint64_t key, const cq::IfValue* param, const cq::IfValue** ret, std::uint64_t& ret_code) override
        {
            switch (id)
            {
            case cq::plugin_msg::SOFTWARE_CHANNEL_QUERY_ACTION:
            {
                cq::QuerySoftwareChannelAction telegram;
                if (parseXMLValue(param, telegram))
                {
                    ...
                    {
                        input_channel_data.push_back({ a_channel.channel_id, a_channel.data_format, cq::Timebase() });
                    }

                    cq::QuerySoftwareChannelActionResponse response;
            ...
            case cq::plugin_msg::SOFTWARE_CHANNEL_CREATE:
            {
                cq::CreateSoftwareChannel telegram;
                ...
                if (parseXMLValue(param, telegram))
                {
                    ...
                    cq::framework::SoftwareChannelInstance::InitParams init_params{input_channel_data, telegram.m_properties};
                    auto init_result = std::static_pointer_cast<cq::framework::SoftwareChannelInstance>(instance)->init(init_params);
                    if (init_result.m_success)
                    {
                        ...
                        if (ret)
                        {
                            cq::CreateSoftwareChannelResponse response;
                            for (const auto& ch : instance->getOutputChannels())
                            {
                                ...
                                if (init_result.m_channel_list_action ==
                                    cq::framework::SoftwareChannelInstance::InitResult::ChannelListAction::SHOW_DETAILS_OF_FIRST_CHANNEL)
                                {
                            ...
                            if (init_result.m_channel_list_action ==
                                cq::framework::SoftwareChannelInstance::InitResult::ChannelListAction::SHOW_DETAILS_OF_SPECIFIED_CHANNEL)
                            {
                                response.m_show_channel_details = true;
            ...
            case cq::plugin_msg::DELETE_PLUGIN_CHANNELS:
            {
                cq::ChannelList channel_list;
                if (parseXMLValue(param, channel_list))
                {
                    ...
                    std::transform(channel_list.m_channels.begin(), channel_list.m_channels.end(), std::back_inserter(channels),
                        [](const cq::ChannelList::ChannelInfo& channel)
                        {
            ...
            case cq::plugin_msg::PLUGIN_CHANNEL_IDS_CHANGED:
            {
                cq::ChannelMappingTelegram<std::uint64_t> telegram;
            ...
            case cq::plugin_msg::PLUGIN_RESET:
            {
            ...
            case cq::plugin_msg::PLUGIN_LOAD_CONFIGURATION:
            {
                cq::UpdateChannelsTelegram telegram;
                if (parseXMLValue(param, telegram))
                {
                    cq::ChannelMappingTelegram<std::uint32_t>::MapType id_map_root_channels;
                    if (createInstancesfromTelegram(telegram, id_map_root_channels))
                    {
                        cq::ChannelMappingTelegram<std::uint32_t> cm;
                        ...
                        for (auto instance : m_instances)
                        {
                            ...
                            const auto original_root_id_it =
                                std::find_if(id_map_root_channels.begin(), id_map_root_channels.end(),
                                    [instance_root_id](
                                        const cq::ChannelMappingTelegram<std::uint32_t>::MapType::value_type& an_id_mapping)
                                    {
                            ...
                            if (original_root_id_it != id_map_root_channels.end())
                            {
                                ...
                                cq::UpdateChannelsTelegram instance_telegram;
                                ...
                                cq::ChannelMappingTelegram<std::uint32_t>::MapType id_map;
            ...
            case cq::plugin_msg::PLUGIN_LOAD_FINISH:
            {
            ...
            case cq::plugin_msg::NOTIFY_EVENT:
            {
                switch (key)
                {
                case cq::EventIds::EVENT_ID_TOPOLOGY_CHANGED:
                {
                ...
                case cq::EventIds::EVENT_ID_LICENSE_CHANGED:
        ...
        std::uint64_t forwardCustomRequest(SoftwareChannelInstanceRequestFunction func, const cq::PropertyList& params, cq::PropertyList& return_list)
        {
