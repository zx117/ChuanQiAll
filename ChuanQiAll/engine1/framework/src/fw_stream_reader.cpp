...
namespace framework
{
    ...
    void StreamReader::addDataRegion(const cq::DataRegion& region)
    {
    ...
    {
        return createChannelIterator(channel_id, cq::Interval<std::uint64_t>(0, std::numeric_limits<std::uint64_t>::max()));
    }

    StreamIterator StreamReader::createChannelIterator(std::uint64_t channel_id, const cq::Interval<std::uint64_t>& interval) const
    {
    ...
    void StreamReader::updateStreamIterator(std::uint64_t channel_id, StreamIterator& iterator, const cq::Interval<std::uint64_t>& interval) const
    {
        ...
        for (auto it_block = blocks_begin; it_block != blocks_end; ++it_block)
        {
            ...
            for (const auto& bcd : block_descriptor.m_block_channels)
            {
                ...
                {                    
                    CQ_ASSERT_EQUAL(bcd.m_offset % 8, 0);
        ...
        CQ_UNUSED(sample_count);
        CQ_ASSERT_EQUAL(iterator.getTotalSampleCount(), sample_count);
    }
