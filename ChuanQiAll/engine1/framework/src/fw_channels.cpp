...
namespace framework
{
    ...
    PluginChannel::PluginChannel(std::uint32_t local_id, IfPluginChannelChangeListener* change_listener, cq::IfHost* host)
    ...
    PluginChannel& PluginChannel::setSampleFormat(
        cq::ChannelDataformat::SampleOccurrence occurrence, cq::ChannelDataformat::SampleFormat format, std::uint32_t dimension)
    {
    ...
    PluginChannel& PluginChannel::setSimpleTimebase(double frequency)
    {
        if (m_change_listener && m_change_listener->configChangeAllowed())
        {
            ...
            {
                setSamplerate(cq::Scalar(frequency, "Hz"));
            }
    ...
    PluginChannel &PluginChannel::setRange(const cq::Range& range)
    {
        if (m_change_listener && m_change_listener->configChangeAllowed())
        {
    ...
    PluginChannel& PluginChannel::setSamplerate(const cq::Scalar& sample_rate)
    {
        if (m_change_listener && m_change_listener->configChangeAllowed())
        {
    ...
    PluginChannel& PluginChannel::addProperty(const std::string& name, const cq::Property& prop)
    {
        if (m_change_listener && m_change_listener->configChangeAllowed())
        {
    ...
    PluginChannel& PluginChannel::replaceProperty(const std::string& name, ChannelPropertyPtr prop)
    {
        ...
        CQ_ASSERT(getProperty(name));
        CQ_ASSERT(std::find_if(m_properties.begin(), m_properties.end(), property_matcher) != m_properties.end());
    ...
    {
        if (const auto name = getChannelParam<cq::IfStringValue>("Name"))
        {
    ...
    {
        CQ_ASSERT(std::find(m_input_channels.begin(), m_input_channels.end(), ch_id) == m_input_channels.end());
        if (m_change_listener) m_change_listener->onTaskInputChannelsChanged(this);
    ...
    {
        CQ_ASSERT(std::find(m_output_channels.begin(), m_output_channels.end(), ch) == m_output_channels.end());
    ...
    void PluginTask::removeOutputChannel(PluginChannelPtr ch)
    {
        ...
        CQ_ASSERT(it != m_output_channels.end());
    ...
    void PluginChannels::setHost(cq::IfHost* host)
    {
        m_host = host;
    }
    ...
    void PluginChannels::synchronize(bool register_tasks)
    {
        ...
        if (m_channels_dirty)
        {
            cq::UpdateChannelsTelegram telegram;
            telegram.m_channels.reserve(m_channels.size());
            for (const auto& channel : m_channels)
            {
                CQ_ASSERT(channel.second->m_channel_info.m_local_parent_id == (channel.second->m_local_parent ? channel.second->m_local_parent->getLocalId() : -1));
                telegram.appendChannel(channel.second->m_channel_info);
            }
            telegram.m_list_topology = m_list_topology;
            {
                auto xml = telegram.generate();
                m_host->messageSyncData(cq::host_msg::SET_PLUGIN_OUTPUT_CHANNELS, 0, xml.c_str(), xml.size() + 1, nullptr);
            }
        }
        if (!m_properties_dirty.empty())
        {
            cq::UpdateConfigTelegram telegram;
            ...
            {
                auto xml = telegram.generate();
                m_host->messageSyncData(cq::host_msg::SET_PLUGIN_CONFIGURATION, 0, xml.c_str(), xml.size() + 1, nullptr);
            }
        }
    ...
    std::set<PluginTaskPtr> PluginChannels::getAffectedTasks(const cq::UpdateConfigTelegram& request)
    {
    ...
    std::uint64_t PluginChannels::processConfigUpdate(const cq::UpdateConfigTelegram& request)
    {
        cq::UpdateConfigTelegram response;
    ...
    std::uint64_t PluginChannels::processDataFormatChange(const cq::ChannelDataformatTelegram& request)
    {
        return processInputChannelChange({request.channel_id});
    }

    uint64_t PluginChannels::processInputChannelConfigChange(const cq::ChannelConfigChangedTelegram& telegram)
    {
    ...
    uint64_t PluginChannels::reserveChannelIds(const cq::ChannelList& telegram)
    {
        m_reseved_ids.clear();
    ...
    void PluginChannels::registerTask(PluginTask& task)
    {
        ...
        {
            cq::AddAcquisitionTaskTelegram telegram;
            ...
            m_host->messageSync(cq::host_msg::ACQUISITION_TASK_ADD, 0, xml_msg.get());
        }
    }
    ...
    void PluginChannels::unregisterTask(PluginTask& t)
    {
        if (t.m_registered)
        {
            m_host->messageSync(cq::host_msg::ACQUISITION_TASK_REMOVE, t.m_id, nullptr);
    ...
    std::uint64_t PluginChannels::pluginMessage(cq::PluginMessageId id, std::uint64_t key, const cq::IfValue* param, const cq::IfValue** ret)
    {
        CQ_UNUSED(ret);
        try
        {
            switch (id)
            {
            case cq::plugin_msg::ACQUISITION_TASK_INIT_TIMEBASES:
                ...
                break;
            case cq::plugin_msg::ACQUISITION_TASK_START_PROCESSING:
                if (auto task = findTask(key))
                {
                    if (task->m_worker)
                    {
                ...
                break;
            case cq::plugin_msg::ACQUISITION_TASK_STOP_PROCESSING:
                if (auto task = findTask(key))
                {
                    if (task->m_worker)
                    {
                ...
                break;
            case cq::plugin_msg::ACQUISITION_TASK_PROCESS:
                if (auto task = findTask(key))
                {
                    if (task->m_worker)
                    {
                        auto process_xml = cq::value_cast<const cq::IfXMLValue>(param);
                ...
                break;
            case cq::plugin_msg::PLUGIN_CONFIGURATION_CHANGE_REQUEST:
                if (auto change_xml = cq::value_cast<const cq::IfXMLValue>(param))
                {
                    cq::UpdateConfigTelegram update_telegram;
                ...
                return cq::error_codes::INVALID_INPUT_PARAMETER;
            case cq::plugin_msg::PLUGIN_CHANNEL_DATAFORMAT_CHANGED:
            {
                cq::ChannelDataformatTelegram telegram;
            ...
            return cq::error_codes::INVALID_INPUT_PARAMETER;
            case cq::plugin_msg::NOTIFY_CHANNEL_CONFIG_CHANGED:
            {
                cq::ChannelConfigChangedTelegram telegram;
            ...
            case cq::plugin_msg::PLUGIN_RESERVE_CHANNEL_IDS:
            {
                cq::ChannelList telegram;
                if (parseXMLValue(param, telegram))
                {
            ...
            default:
                return cq::error_codes::NOT_IMPLEMENTED;
            }
            return cq::error_codes::NOT_IMPLEMENTED;

        }
        catch (const cq::framework::CallbackError& e)
        {
            return e.getErrorCode();
        }
        catch (const std::exception& e)
        {
            LOG_ERROR("Unhandled exception during 'pluginMessage " << id << "' : " << e.what());
        }
        catch (...)
        {
            LOG_ERROR("Unhandled exception during 'pluginMessage " << id << "'");
        }
