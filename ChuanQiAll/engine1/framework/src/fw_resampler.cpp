...
std::uint64_t Resampler::addSamples(cq::IfHost* host, std::uint32_t local_channel_id, double last_sample_timestamp, const double* data, std::size_t num_samples)
{
    if (num_samples == 0)
    {
    ...
    if (!m_input_buffer.empty())
    {
        ...
        CQ_ASSERT_GTE(first_sample_timestamp, 0);
    }
    ...
    std::uint64_t result = cq::error_codes::OK;
    if (last_sample_int > m_actual_scnt)
    {
        ...
        *reinterpret_cast<std::uint64_t*>(m_output_buffer.data()) = m_actual_scnt;
        ...
        result = host->messageSyncData(cq::host_msg::ADD_CONTIGUOUS_SAMPLES, local_channel_id, m_output_buffer.data(), m_output_buffer.size() * sizeof(double), nullptr);
