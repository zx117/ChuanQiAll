#include "fw_software_channel_instance.h"
...
#include "fw_channels.h"
#include "fw_exceptions.h"
...
namespace framework
{
    ...
    std::map<uint64_t, cq::framework::StreamIterator> SoftwareChannelInstance::createChannelIterators(
        const std::vector<cq::StreamDescriptor>& stream_descriptor,
        const cq::IfDataBlockList* block_list)
    {
        cq::DataRegions data_regions;

        for (const auto& sd : stream_descriptor)
        {
            for (auto& channel : sd.m_channel_descriptors)
            {
                data_regions.m_data_regions.push_back(DataRegion(channel.m_channel_id, cq::Interval<std::uint64_t>(0, std::numeric_limits<std::uint64_t>::max())));
            }
        }
        return createChannelIterators(stream_descriptor,
                                      block_list,
                                      cq::Interval<double>(0, std::numeric_limits<double>::max()),
                                      data_regions);
    }
    ...
    std::map<uint64_t, cq::framework::StreamIterator> SoftwareChannelInstance::createChannelIterators(
        const std::vector<cq::StreamDescriptor>& stream_descriptor,
        const cq::IfDataBlockList* block_list,
        const cq::Interval<double>& interval,
        const cq::DataRegions& data_regions)
    {

        std::map<uint64_t, cq::framework::StreamIterator> iterators;

        auto block_list_descriptor_xml = cq::ptr(block_list->getBlockListDescription());
        ...
        {
            auto block = cq::ptr(block_list->getBlock(i));
        ...
        for (const auto& sd : stream_descriptor)
        {
            ...
            for (auto& channel : sd.m_channel_descriptors)
            {
                cq::Interval<std::uint64_t> channel_interval(0, std::numeric_limits<std::uint64_t>::max());
    ...
    SoftwareChannelInstance::InitResult SoftwareChannelInstance::init(const InitParams& params)
    {
        ...
        {
            LOG_ERROR("Unhandled exception during 'init': " << e.what());
        }
        catch (...)
        {
            LOG_ERROR("Unhandled exception duriong 'init'");
        }
        return { false };
    }
    ...
    void SoftwareChannelInstance::initInstance(cq::IfHost* host)
    {
        try
        {
            m_host = host;
            ...
            channel->setSampleFormat(
                cq::ChannelDataformat::SampleOccurrence::NEVER,
                cq::ChannelDataformat::SampleFormat::NONE,
                0);
        ...
        {
            LOG_ERROR("Unhandled exception during 'initInstance': " << e.what());
        }
        catch (...)
        {
            LOG_ERROR("Unhandled exception during 'initInstance'");
        }
    }
    ...
    void SoftwareChannelInstance::onInitTimebases(cq::IfHost* host, std::uint64_t token)
    {
        CQ_UNUSED(token);
        try
        {
        ...
        {
            LOG_ERROR("Unhandled exception during 'initTimebases': " << e.what());
        }
        catch (...)
        {
            LOG_ERROR("Unhandled exception during 'initTimebases'");
        }
    }

    void SoftwareChannelInstance::onStartProcessing(cq::IfHost* host, std::uint64_t token)
    {
        CQ_UNUSED(token);
        ...
        try
        {
            if (!m_input_channel_proxies.empty())
            {
                ...
                if (m_dataset_descriptor && m_data_request_type == DataRequestType::STREAM)
                {
                    ...
                    auto msg = host->createValue<cq::IfXMLValue>();
                    msg->set(req.generate().c_str());
                    host->messageAsync(cq::host_msg::DATA_START_REQUEST, 0, msg.get());
                }
            }
        ...
        {
            LOG_ERROR("Unhandled exception during 'prepareProcessing': " << e.what());
        }
        catch (...)
        {
            LOG_ERROR("Unhandled exception during 'prepareProcessing'");
        }
        throw cq::framework::CallbackError(cq::error_codes::UNHANDLED_EXCEPTION);
    }

    void SoftwareChannelInstance::onStopProcessing(cq::IfHost *host, std::uint64_t token)
    {
        CQ_UNUSED(token);
        ...
        try
        {
            if (m_dataset_descriptor)
            {
                if (m_data_request_type == DataRequestType::STREAM)
                {
                    ...
                    auto msg = host->createValue<cq::IfXMLValue>();
                    msg->set(req.generate().c_str());
                    host->messageAsync(cq::host_msg::DATA_STOP_REQUEST, 0, msg.get());
                }
        ...
        {
            LOG_ERROR("Unhandled exception during 'stopProcessing': " << e.what());
        }
        catch (...)
        {
            LOG_ERROR("Unhandled exception during 'stopProcessing'");
        }
        throw cq::framework::CallbackError(cq::error_codes::UNHANDLED_EXCEPTION);
    }

    std::map<uint64_t, cq::framework::StreamIterator> SoftwareChannelInstance::getSamplesAt(double time)
    {
        if(m_dataset_descriptor)
        {
            ...
            if (xml_msg)
            {
                ...
                if (auto block_list = cq::value_cast<cq::IfDataBlockList>(response))
                {
    ...
    void SoftwareChannelInstance::onProcess(cq::IfHost *host, std::uint64_t token, const cq::IfXMLValue* param)
    {
        CQ_UNUSED(token);

        std::uint64_t ret = cq::error_codes::OK;
        cq::AcquisitionTaskProcessTelegram telegram;
        if (param)
        {
        ...
        if (m_dataset_descriptor && telegram.m_start.timestampValid() && telegram.m_end.timestampValid())
        {
            cq::DataRegions data_regions;
            {
                ...
                host->messageSync(cq::host_msg::DATA_REGIONS_READ, 0, xml_msg.get(), &data_regions_result);
            ...
            if (0 != host->messageSync(cq::host_msg::DATA_READ, 0, xml_msg.get(), &response))
            {
                return;
            }
            ...
            if (auto block_list = cq::value_cast<cq::IfDataBlockList>(response))
            {
                ...
                context.m_channel_iterators = createChannelIterators(m_dataset_descriptor->m_stream_descriptors,
                                                                     block_list,
                                                                     cq::Interval<double>(context.m_window.first, context.m_window.second),
                                                                     data_regions);
        ...
        else if (m_dataset_descriptor && m_data_request_type == DataRequestType::STREAM)
        {
            ...
            while (current_time < max_time)
            {
                ...
                cq::MessageReturnValueHolder<cq::IfDataBlockList> block_list;
                if (0 != host->messageSync(cq::host_msg::DATA_READ, 0, xml_msg.get(), block_list.data()))
                {
                    throw cq::framework::CallbackError(cq::error_codes::INTERNAL_ERROR);
                }
                ...
                if (block_list.valid())
                {
                    ...
                    {
                        LOG_ERROR("Unhandled exception during 'process': " << e.what());
                        ret = cq::error_codes::UNHANDLED_EXCEPTION;
                    }
                    catch (...)
                    {
                        LOG_ERROR("Unhandled exception during 'process'");
                        ret = cq::error_codes::UNHANDLED_EXCEPTION;
                    }
        ...
        if (ret != cq::error_codes::OK)
        {
            throw cq::framework::CallbackError(ret);
        }
    }

    void SoftwareChannelInstance::onChannelConfigChanged(cq::IfHost* host, std::uint64_t token)
    {
        CQ_UNUSED(host);
        CQ_UNUSED(token);
    ...
    void SoftwareChannelInstance::setupDataRequest(cq::IfHost *host)
    {
        ...
        const cq::IfValue* group_add_result = nullptr;
        host->messageSync(cq::host_msg::DATA_GROUP_ADD, 0, xml_msg.get(), &group_add_result);

        const cq::IfXMLValue* group_add_result_xml = cq::value_cast<cq::IfXMLValue>(group_add_result);
        if (group_add_result)
        {
        ...
        for(auto& channel : getOutputChannels())
        {
            for(auto& property : channel->getProperties())
            {
                ...
                else if(auto channel_list_property = std::dynamic_pointer_cast<EditableChannelIDListProperty>(property.second))
                {
                    ...
                    channel_list_property->setValue(cq::ChannelIDList(new_input_channel_ids));
                }
            }
        }
    ...
    void SoftwareChannelInstance::enqueueRequest(uint16_t id, const PropertyList &params)
    {
        ...
        auto value = cq::framework::utils::convertToXMLValue(getHost(), properties);
        getHost()->messageAsync(cq::plugin_msg::CUSTOM_QML_REQUEST, id, value);
    }

    void SoftwareChannelInstance::fetchInputChannels()
    {
        clearRequestedInputChannels();
        std::set<cq::ChannelID> all_input_channels;
    ...
    bool SoftwareChannelInstance::checkInputChannelFormats(const std::set<cq::ChannelDataformat::SampleValueType>& allowed_value_types,
                                    const std::set<cq::ChannelDataformat::SampleOccurrence>& allowed_occurence)
    {
    ...
    void SoftwareChannelInstance::createChannelsFromTelegram(const UpdateChannelsTelegram& request, std::map<uint32_t, uint32_t>& channel_id_map)
    {
        for (auto &requested_channel : request.m_channels)
        {
            ...
            if(!ch)
            {
                ...
                {
                    CQ_ASSERT_FAIL("Unable to determine Parent");
                }
            }
            else
            {
        ...
        for (auto &requested_channel : request.m_channels)
        {
            ...
            if(const auto& output_channel = getOutputChannel(local_id))
            {
                ...
                for (const auto& property : requested_channel.m_channel_config.m_properties)
                {
                    ...
                    {
                        CQ_VERIFY(destination_property->update(property));
                    }
                    else
                    {
                        CQ_ASSERT_FAIL("Unable to configure property %s", property_name.c_str());
                    }
                }
            }
        }
    ...
    PluginChannelPtr SoftwareChannelInstance::getOutputChannel(uint32_t local_id) const
    {
        auto match = std::find_if(m_output_channels.begin(), m_output_channels.end(),
                                  [&local_id](const cq::framework::PluginChannelPtr& output_channel)
        {
    ...
    {
        auto match = std::find_if(m_output_channels.begin(), m_output_channels.end(),
                                  [&key](const cq::framework::PluginChannelPtr& output_channel)
        {
    ...
    cq::IfHost* SoftwareChannelInstance::getHost()
    {
        return m_host;
    }
