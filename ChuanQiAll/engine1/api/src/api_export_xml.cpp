...
namespace
{
    typedef boost::bimap<cq::RegisterExport::StartExportAction, std::string> StartExportActionBiMap;
    static const StartExportActionBiMap START_EXPORT_OPTION_MAP = boost::assign::list_of<StartExportActionBiMap::relation>
        (cq::RegisterExport::StartExportAction::SELECT_FILE, "SELECT_FILE")
        (cq::RegisterExport::StartExportAction::SELECT_DIRECTORY, "SELECT_DIRECTORY")
        (cq::RegisterExport::StartExportAction::NONE, "NONE");
}
...
namespace cq
{
    ...
    bool RegisterExport::parse(const boost::string_view& xml_string)
    {
        ...
        if (!cq::strequal(node.name(), "RegisterExporter"))
        {
            return false;
        }
        ...
        if (m_success)
        {
            ...
            if (!m_channel_warnings.empty())
            {
                ...
                for (const auto& channel : m_channel_warnings)
                {
                    ...
                    xpugi::setText(ch_warning.append_child("ErrorMessage"), cq::error_codes::toString(channel.error_code));
                }
            }
        }
        else
        {
            ...
            if (!m_channel_errors.empty())
            {
                ...
                for (const auto& channel : m_channel_errors)
                {
                    ...
                    //xpugi::setText(ch_error.append_child("ErrorMessage"), cq::error_codes::toString(channel.error_code));
                }
            }
        }
        return xpugi::toXML(doc);
