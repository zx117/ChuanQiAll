...
namespace cq
{
    ...
    Property PropertyList::getPropertyByName(const std::string& name) const
    {
        const auto match =
                std::find_if(
                    cbegin(), cend(),
                    [&name] (const cq::Property& property)
                    {
    ...
    bool PropertyList::getBool(const std::string& name) const
    {
        ...
        if (prop.getType() == cq::Property::BOOLEAN)
        {
    ...
    {
        auto prop = getPropertyByName(name);
        if (prop.getType() == cq::Property::INTEGER || prop.getType() == cq::Property::INTEGER64)
        {
    ...
    {
        auto prop = getPropertyByName(name);
        if (prop.getType() == cq::Property::UNSIGNED_INTEGER || prop.getType() == cq::Property::UNSIGNED_INTEGER64)
        {
    ...
    {
        auto prop = getPropertyByName(name);
        if (prop.getType() == cq::Property::FLOATING_POINT_NUMBER)
        {
    ...
    {
        auto prop = getPropertyByName(name);
        if (prop.getType() == cq::Property::STRING)
        {
    ...
    cq::Scalar PropertyList::getScalar(const std::string& name) const
    {
        auto prop = getPropertyByName(name);
        if (prop.getType() == cq::Property::SCALAR)
        {
            return prop.getScalarValue();
        }
        return {};
    }
    cq::Scalar PropertyList::getScalar(std::size_t idx) const
    {
        auto prop = getProperty(idx);
        if (prop.getType() == cq::Property::SCALAR)
        {
    ...
    {
        auto prop = getPropertyByName(name);
        if (prop.getType() == cq::Property::CHANNEL_ID)
        {
    ...
    {
        auto prop = getPropertyByName(name);
        if (prop.getType() == cq::Property::PROPERTY_LIST)
        {
