...
namespace cq
{
    ...
    bool UpdateConfigTelegram::parse(const boost::string_view& xml_string)
    {
        ...
        if (status.status == pugi::status_ok)
        {
            ...
            if (strcmp(request_node.name(), "UpdateConfig") != 0)
                return false;
            auto version = cq::getProtocolVersion(request_node);
    ...
    std::string UpdateConfigTelegram::generate() const
    {
        ...
        cq::setProtocolVersion(request_node, cq::Version(1, 0));
...
/*
    ...
    uint64_t ConfigHelper::pluginMessage(PluginMessageId id, uint64_t key, const IfValue *param, const IfValue **ret)
    {
        switch (id)
        {
        case cq::plugin_msg_sync::PLUGIN_CONFIGURATION_CHANGE_REQUEST:
    ...
    {
        if (auto change_xml = value_cast<const cq::IfXMLValue>(param))
        {
...
*/
    ...
    void UpdateConfigTelegram::ChannelConfig::appendProperties(pugi::xml_node channel_node) const
    {
        ...
        for (const auto& constraint : m_constraints)
        {
            ...
            {
                CQ_ASSERT_FAIL("Orphaned constraints are not supported");
            }
        }
    }
    ...
    void UpdateConfigTelegram::Constraint::appendTo(pugi::xml_node parent) const
    {
        switch (getType())
        {
            ...
            case Constraint::RANGE:
            {
                ...
                if (pmin.getType() == cq::Property::FLOATING_POINT_NUMBER)
                {
                ...
                {
                    CQ_ASSERT_FAIL("Unimplemented range type");
                }
            } break;
            ...
            default:
                CQ_ASSERT_FAIL("Unimplemented constraint type");
                break;
        }
    }
