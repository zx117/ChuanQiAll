...
namespace
{
    ...
    bool toLocalChannelIDMap(const pugi::xml_node& node,
        typename cq::ChannelMappingTelegram<IdType>::MapType& channel_id_map)
    {
        if (0 == std::strcmp("ChannelIDMap", node.name()))
        {
            ...
            for (const auto channel_mapping_node : channel_mapping_nodes)
            {
                ...
                CQ_ASSERT_LTE(first, std::numeric_limits<IdType>::max());
                CQ_ASSERT_LTE(second, std::numeric_limits<IdType>::max());
    ...
    template <typename IdType>
    bool toLocalChannelIDMapXML(const typename cq::ChannelMappingTelegram<IdType>::MapType& channel_id_map,
        pugi::xml_node& node)
    {
        if (node) // <-- parent
        {
