...
namespace
{
    ...
    std::string stringFormatToString(cq::Property::StringFormat f)
    {
        switch (f)
        {
            case cq::Property::STRING_MULTILINE: return "MULTI";
            case cq::Property::STRING_RST: return "RST";
            case cq::Property::STRING_XML: return "XML";
            case cq::Property::STRING_PLAIN: return "";
        }
        return "";
    }

    cq::Property::StringFormat parseStringFormat(const std::string& fmt)
    {
        if (fmt == "MULTI")
        {
            return cq::Property::STRING_MULTILINE;
        }
        if (fmt == "XML")
        {
            return cq::Property::STRING_XML;
        }
        if (fmt == "RST")
        {
            return cq::Property::STRING_RST;
        }
        else
        {
            return cq::Property::STRING_PLAIN;
        }
    }
...
namespace cq
{
    ...
    {
        if (!cq::strequal(tree.name(), getNodeName().c_str()) || tree.type() != pugi::node_element)
        {
            LOG_ERROR("Error parsing Property");
            return false;
        }
        pugi::xml_attribute attr = tree.attribute("name");
        if (!attr)
        {
            LOG_ERROR("Error parsing name attribute from Property");
            return false;
        }
        ...
        if (!ret)
        {
            LOG_ERROR("Error: Property " << m_name << " has no supported type defining node");
        }
        return ret;
    }
    ...
    bool Property::readValue(pugi::xml_node type_node, const Version& version)
    {
        ...
        switch (type)
        {
            case UNKNOWN:
            {
                LOG_DEBUG("Warning: Property has unknown type " << type_name);
                break;
            }
            case STRING:
            {
    ...
    std::string Property::toXMLType(Type type) const
    {
        ...
        default:
            LOG_WARN("Unknown property data type " << m_type);
            return {};
        }
    }
    ...
    Property::Type Property::fromXMLType(const std::string& xml_type) const
    {
        ...
        LOG_WARN("Unknown property data type " << xml_type);
    ...
    bool Property::parseEnum(const pugi::xml_node& type_node)
    {
        ...
        if (!attr)
        {
            LOG_ERROR("Error parsing enum attribute from Property")
                return false;
        }
    ...
    PointList Property::parsePointListNode(const pugi::xml_node& type_node)
    {
        ...
        if (list_nodes.begin() != list_nodes.end())
        {
            ...
            for (auto node : list_node.children())
            {
                if (node.type() == pugi::node_element)
                {
                    if (cq::strequal(node.name(), "Point"))
                    {
                        bool okay = false;
                        ...
                        if (!okay)
                        {
                            LOG_WARN("PointList::Point has incompatible content")
                        }
                        point_list.m_values.push_back(std::make_pair(x, y));
                    }
                    else
                    {
                        LOG_DEBUG("Warning: " << list_node.name() << " has unknown node " << node.name())
                    }
                }
            }
        }
    ...
    ChannelIDList Property::parseChannelIdListNode(const pugi::xml_node& type_node)
    {
        ...
        {
            LOG_DEBUG("Warning: " << type_node.name() << " has no ChannelIDList node");
        }
        return list;
    }
    ...
    std::string Property::valueToString() const
    {
        switch (m_type)
        {
            case SCALAR:
            {
                cq::Scalar scalar = getScalarValue();
            ...
            case FLOATING_POINT_NUMBER_LIST:
            {
                cq::DoubleList list = getDoubleListValue();
            ...
            case STRING_LIST:
            {
                cq::StringList list = getStringListValue();
                std::uint32_t num_of_elems = static_cast<std::uint32_t>(list.m_values.size());
                std::uint32_t index = 0;
                std::string ret_string{};
            ...
            case PROPERTY_LIST:
            {
                cq::PropertyList list = getPropertyListValue();
            ...
            case POINT_LIST:
            {
                cq::PointList list = getPointListValue();
                std::uint32_t num_of_elems = static_cast<std::uint32_t>(list.m_values.size());
                std::uint32_t index = 0;
                std::string ret_string{};
                ret_string.append("(");
